...
smartcar::tools::Traj_Extractor::getInstance()
{
    if (!instance)
    {
...
bool smartcar::tools::Traj_Extractor::set_folder(const std::string &input_folder, const std::string &output_folder)
{
    ...
    if (!smartcar::utils::endswith(input_folder, "/"))
        _input_folder += "/";
    if (!smartcar::utils::endswith(output_folder, "/"))
        _output_folder += "/";
}
...
bool smartcar::tools::Traj_Extractor::write_back()
{
    return on_write_back();
}

void smartcar::tools::Traj_Extractor::reload_map()
{
    load_map();
}
...
void smartcar::tools::Traj_Extractor::init_ros()
{
    ...
    sub_pose = nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/initialpose", 1, boost::bind(&smartcar::tools::Traj_Extractor::pose_cb, this, _1));
...
bool smartcar::tools::Traj_Extractor::pose_cb(
...
bool smartcar::tools::Traj_Extractor::insert_pose(
...
bool smartcar::tools::Traj_Extractor::smooth_path(double weight_data,
                                                  ...
                                                  double tolerance)
{
    ...
    while (change >= _tolerance)
    {
        ...
        for (int i = std::max(size - 50, 1); i < size - 1; i++) //
        {
            ...
            current_path.poses[i].pose.position.x += weight_smooth * (current_path.poses[i - 1].pose.position.x + current_path.poses[i + 1].pose.position.x - (2.0 * current_path.poses[i].pose.position.x));
            current_path.poses[i].pose.position.y += weight_smooth * (current_path.poses[i - 1].pose.position.y + current_path.poses[i + 1].pose.position.y - (2.0 * current_path.poses[i].pose.position.y));
...
double
smartcar::tools::Traj_Extractor::distance2points(const geometry_msgs::PoseStamped p1,
...
bool smartcar::tools::Traj_Extractor::on_create_trajectory(traj_info &type)
{
    ...
    else if (type.type == traj_type::LANE)
    {
        ...
        {
            path_length +=
                distance2points(current_path.poses[i], current_path.poses[i + 1]);
        }
        ...
        std::cout << "Insert lane id: " << type.id << " with " << size << " points"
                  << std::endl;
    }
    else if (traj_type::CROSS)
    {
        ...
        std::cout << "Insert cross id: " << type.id << " with " << size << " points"
                  << std::endl;
    }
...
bool smartcar::tools::Traj_Extractor::on_write_back()
{
    ...
    std::cout << "********* The Lane list is as followed *************"
              << std::endl;
    for (int i = 0; i < 101; i++)
    {
        if (lane_list[i].path.poses.empty())
            continue;
        // pre
        ...
        for (int j = 0; j < before_size; j++)
            std::cout << lane_list[i].pre_ids[j] << " ";
        std::cout << std::endl;

        // next
        int before_size1 = lane_list[i].next_ids.size();
        std::cout << "(next)before size: " << before_size1 << std::endl;
        for (int j = 0; j < before_size1; j++)
            std::cout << lane_list[i].next_ids[j] << " ";
        std::cout << std::endl;
    ...
    std::cout << "********* The Cross list is as followed *************"
              << std::endl;
    for (int i = 0; i < 101; i++)
    {
        if (cross_list[i].path.poses.empty())
            continue;
        // pre
        ...
        std::cout << "(pre)before size: " << before_size << std::endl;
        for (int j = 0; j < before_size; j++)
            std::cout << cross_list[i].pre_ids[j] << " ";
        std::cout << std::endl;
        ...
        std::cout << "(next)before size: " << before_size1 << std::endl;
        for (int j = 0; j < before_size1; j++)
            std::cout << cross_list[i].next_ids[j] << " ";
        std::cout << std::endl;
...
bool smartcar::tools::Traj_Extractor::write_func(const std::string path_type,
                                                 const int id)
{
    ...
    if (std::strcmp(path_type.c_str(), "lane") == 0)
    {
        ...
        for (int i = 0; i < lane_list[id].pre_ids.size(); i++)
        {
            ...
            if (i < lane_list[id].pre_ids.size() - 1)
                f << ",";
        }
        f << std::endl;
        ...
        for (int i = 0; i < lane_list[id].next_ids.size(); i++)
        {
            ...
            if (i < lane_list[id].next_ids.size() - 1)
                f << ",";
        }
        f << std::endl;
        ...
        for (size_t i = 0; i < len_lane - 1; i++)
        {
            ...
            double yaw = std::atan2(p_n.pose.position.y - p_c.pose.position.y,
                                    p_n.pose.position.x - p_c.pose.position.x);
        ...
        lane_list[id].path.poses[len_lane - 1].pose.orientation.x =
            lane_list[id].path.poses[len_lane - 2].pose.orientation.x;
        lane_list[id].path.poses[len_lane - 1].pose.orientation.y =
            lane_list[id].path.poses[len_lane - 2].pose.orientation.y;
        lane_list[id].path.poses[len_lane - 1].pose.orientation.z =
            lane_list[id].path.poses[len_lane - 2].pose.orientation.z;
        lane_list[id].path.poses[len_lane - 1].pose.orientation.w =
            lane_list[id].path.poses[len_lane - 2].pose.orientation.w;
    ...
    else
    {
        ...
        for (int i = 0; i < cross_list[id].pre_ids.size(); i++)
        {
            ...
            if (i < cross_list[id].pre_ids.size() - 1)
                f << ",";
        }
        f << std::endl;
        ...
        for (int i = 0; i < cross_list[id].next_ids.size(); i++)
        {
            ...
            if (i < cross_list[id].next_ids.size() - 1)
                f << ",";
        }
        f << std::endl;
        ...
        cross_list[id].path.poses[len_lane - 1].pose.orientation.x =
            cross_list[id].path.poses[len_lane - 2].pose.orientation.x;
        cross_list[id].path.poses[len_lane - 1].pose.orientation.y =
            cross_list[id].path.poses[len_lane - 2].pose.orientation.y;
        cross_list[id].path.poses[len_lane - 1].pose.orientation.z =
            cross_list[id].path.poses[len_lane - 2].pose.orientation.z;
        cross_list[id].path.poses[len_lane - 1].pose.orientation.w =
            cross_list[id].path.poses[len_lane - 2].pose.orientation.w;
...
bool smartcar::tools::Traj_Extractor::shutdown()
{
    return on_shutdown();
}
